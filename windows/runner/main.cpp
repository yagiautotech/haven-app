                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"Haven"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"HavenMutex");
  HWND handle = FindWindowA(NULL, "Haven");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"Haven"))
    {
      return false;
    }
  ...
  if (!window.Create(L"Haven", origin, size))
  {
    return EXIT_FAILURE;
  }
